pipeline {
    ...
    stages {
        ...
        stage('Build Docker') {
            ...
            steps {
                sh 'docker build --build-arg BACKEND_API=$BACKEND_API --build-arg Get_Synonyms_API_Prefix=$Get_Synonyms_API_Prefix -t  .'
            }
        }
        ...
        stage('Push Docker Image to ECR') {
            steps {
                ...
                withAWS(region: 'ap-southeast-2', credentials: 'AWS_CRED') {
                    sh "aws ecr get-login-password --no-include-email --region ap-southeast-2 | docker login -u AWS -p $(aws ecr get-login-password --region ap-southeast-2) 123436089261.dkr.ecr.ap-southeast-2.amazonaws.com"
